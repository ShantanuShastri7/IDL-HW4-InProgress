...
class ASRTrainer(BaseTrainer):
    ...
    def _train_epoch(self, dataloader):
        ...
        # Determine device type string for autocast
        # Fixes AttributeError: 'str' object has no attribute 'type'
        if isinstance(self.device, torch.device):
            device_type = self.device.type
        else:
            # If it's a string (e.g. "cuda:0"), split to get just "cuda"
        ...
        for i, batch in enumerate(dataloader):
            ...
            # --- FIX: Use the robust device_type variable defined above ---
            ...
            # Initialize scaler if it doesn't exist
            ...
            if (i + 1) % self.config['training']['gradient_accumulation_steps'] == 0:
                ...
                # Handle scheduler stepping (unless it's ReduceLROnPlateau)
    ...
    def _validate_epoch(self, dataloader):
        ...
        for res in results:
            if 'target' in res:
                references.append(res['target'])
                hypotheses.append(res['generated'])
        ...
        with torch.inference_mode():
            for i, batch in enumerate(dataloader):
                ...
                if recognition_config['beam_width'] > 1:
                    ...
                    # Pick best beam (index 0, as they are sorted in generate_beam)
                    seqs = seqs[:, 0, :]
                    scores = scores[:, 0]
                    # --- FIX END ---
                else:
