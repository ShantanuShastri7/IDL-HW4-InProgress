...
class ASRDataset(Dataset):
    def __init__(
        ...
        for i in tqdm(range(self.length)):
            ...
            if self.partition != "test-clean":
                ...
                tokenized = tokenizer.encode("".join(transcript.tolist()))
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.partition != "test-clean":
            ...
            transcript_lengths = [
                i.shape[0] for i in batch_shifted
            ]
        ...
        return padded_feats, padded_shifted, padded_golden, torch.LongTensor(feat_lengths), torch.LongTensor(transcript_lengths) if transcript_lengths is not None else None
