...
def PadMask(padded_input, input_lengths):
    ...
    attn_mask = torch.zeros(N, T, dtype=torch.bool, device=padded_input.device)

    for i in range(N):
        valid_length = input_lengths[i]
        if valid_length < T:
            attn_mask[i, valid_length:] = True
...
def CausalMask(padded_input):
    ...
    """
    T = padded_input.shape[1]  # Sequence length
    attn_mask = torch.zeros(T, T, dtype=torch.bool, device=padded_input.device)

    causal_mask = torch.triu(
        torch.ones(T, T, dtype=torch.bool, device=padded_input.device), diagonal=1
    )
