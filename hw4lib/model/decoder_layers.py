...
class SelfAttentionDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, d_ff: int, dropout: float = 0.1):
        ...
        heads= num_heads
        d_ff = d_ff
        self.self_attn = SelfAttentionLayer(d, heads) # Masked self-attention layer
        self.ffn = FeedForwardLayer(d, d_ff) # Feed-forward network
...
class CrossAttentionDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, d_ff: int, dropout: float = 0.1):
        ...
        # TODO: Initialize the sublayers  
        self.self_attn  = SelfAttentionDecoderLayer(d_model, num_heads, d_ff, dropout) # Masked self-attention layer
